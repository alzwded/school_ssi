...
void config() {  fscanf(configFile, "%d %d", &magic1, &magic2);
...
void encrypt() { ninputText = fread(inputText, sizeof(char), 10 << 20, inFile);
...
void decrypt() { ninputText = fread(inputText, sizeof(char), 10 << 20, inFile);
